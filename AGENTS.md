...
**Stack:** Python 3.11+, Streamlit, Anthropic SDK (Codex Sonnet), ReportLab, Pandas, Matplotlib
...
├── simulador.py        # Supplier behavior simulation (pure logic → Codex API narrative)
├── prompt_engine.py    # Structured prompts for Codex API
...
- Codex API is an amplifier, not a crutch — used only for narrative generation in `simulador.py` and `prompt_engine.py`
...
**Architecture:** Deterministic data layer + Codex API narrative layer.
...
3. Pass structured data to Codex API for natural language narrative
...
## Codex API Usage (prompt_engine.py)

**Model:** `Codex-sonnet-4-20250514` for MVP. Opus as future option in config.
**API key:** loaded from `st.secrets["ANTHROPIC_API_KEY"]` (Streamlit Cloud) or `os.environ["ANTHROPIC_API_KEY"]` (local dev).

**Cost control:**
- Codex API is triggered ONLY by explicit user action ("Generate AI analysis" button)
